    ...
    def _data_callback(self, device_model_instance):
        ...
        for addr in self.imu_addresses:
            if addr in data:
                ...
                # ============================================================
                # 🔧 个别陀螺仪数据处理区域 - 在这里添加自定义数学运算
                # ============================================================
                # 示例: 根据地址对不同 IMU 做不同处理
                # if addr == 0x50:  # IMU1 (杆1)
                #     yaw = yaw * 1.7 # 偏移校正
                # elif addr == 0x51:  # IMU2 (杆2)
                #     roll = -roll  # 翻转
                # elif addr == 0x52:  # IMU3 (机械爪)
                #     yaw = yaw * 1.1  # 缩放
                # elif addr == 0x54:  # IMU4 (手指)
                #     pitch = pitch - 10.0  # 偏移校正
                # ============================================================
